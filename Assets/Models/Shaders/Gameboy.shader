Shader "Gameboy"
{
	Properties
	{
		...
		_Darkest ("Darkest", color) = (0.0588235, 0.21961, 0.0588235)	// Gameboy palette
	...
	SubShader
	{
		...
		Pass
		{
			...
			{
				float4 originalColor = tex2D(_MainTex, i.uv); // get pixel value
				float luma = dot(originalColor.rgb, float3(0.2126, 0.7152, 0.0722)); // calculate pixel brightness using sRGB vector
				float posterized = floor(luma * 4) / (3); // luma, times number of steps (colors), divided by number of steps minus 1
